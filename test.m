...
mapping = JointBayesian(train_x, labels);
[classes, ~, labels] = unique(labels);
nc = length(classes);
train_Intra = zeros(nc * 2, 2);
for i=1:nc
  train_Intra(2*i-1,:) = ...
    randperm(sum(labels == i), 2) + find(labels == i, 1, 'first') - 1;
  train_Intra(2*i,:) = ...
    randperm(sum(labels == i), 2) + find(labels == i, 1, 'first') - 1;
end;
...
for i=1:size(train_Intra,1)
  Dis_train_Intra(i) = ...
    train_x(train_Intra(i,1),:) * mapping.A * train_x(train_Intra(i,1),:)' ...
    + train_x(train_Intra(i,2),:) * mapping.A * train_x(train_Intra(i,2),:)' ...
    - 2 * train_x(train_Intra(i,1),:) * mapping.G * train_x(train_Intra(i,2),:)';
  
  Dis_train_Extra(i) = ...
    train_x(train_Extra(i,1),:) * mapping.A * train_x(train_Extra(i,1),:)' ...
    + train_x(train_Extra(i,2),:) * mapping.A * train_x(train_Extra(i,2),:)' ...
    - 2 * train_x(train_Extra(i,1),:) * mapping.G * train_x(train_Extra(i,2),:)';
end;
group_train = ...
  [ones(size(Dis_train_Intra,1),1);zeros(size(Dis_train_Extra,1),1)];
...
for i=1:3000
  result_Intra(i) = ...
    normX(test_Intra(i,1),:) * mapping.A * normX(test_Intra(i,1),:)' ...
    + normX(test_Intra(i,2),:) * mapping.A * normX(test_Intra(i,2),:)' ...
    - 2 * normX(test_Intra(i,1),:) * mapping.G * normX(test_Intra(i,2),:)';
  
  result_Extra(i) = ...
    normX(test_Extra(i,1),:) * mapping.A * normX(test_Extra(i,1),:)' ...
    + normX(test_Extra(i,2),:) * mapping.A * normX(test_Extra(i,2),:)' ...
    - 2 * normX(test_Extra(i,1),:) * mapping.G * normX(test_Extra(i,2),:)';
end;
